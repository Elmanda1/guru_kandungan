...
@section('content')
                ...
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table m-0 align-middle table-hover">
                            ...
                            <tbody>
                            @if(!empty($topics) && $topics->count() > 0)
                                @foreach($topics as $index => $topic)
                                    <tr class="align-middle">
                                        <th class="text-nowrap text-center">{{ $index + $topics->firstItem() }}</th>
    ...
    @php $topic = session('topic') @endphp
    @if($topic != null)
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <form action="{{ route('topic.update', $topic) }}" method="post">
                        ...
                        <div class="modal-header border-0">
                            <h5 class="modal-title" id="editModalLabel">{{ __('Edit Topic') }}</h5>
        ...
        function handleDelete(id) {
            new Swal({
                ...
                if (result.isConfirmed) {
                    let url = `{{ route('topic.destroy', ['']) }}/${id}`;
